    ...
    def rel(self, x, y):
        ...
        if self.reduction:
            if self.size_average:
                ...
                return torch.sum(diff_norms)/torch.sum(y_norms)
...
class RandomMesh_fromfile(object):
    def __init__(self, grid_data):
        super(RandomMesh_fromfile, self).__init__()

        self.grid = grid_data
        self.grid_sample = self.grid
        self.d = 2
        self.idx = np.array(range(grid_data.shape[0]))
